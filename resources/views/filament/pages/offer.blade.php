    ...
    <div class="fi-section rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10">
        @if ($offerExists)
            <div class="flex flex-col gap-4">
                ...
                @if ($textBlocks > 0)
                    <p class="rounded-lg bg-success-50 p-3 text-sm text-success-700 dark:bg-success-500/10 dark:text-success-300">
                        Клиенты читают <b>текстовую версию</b> договора на странице сайта: PDF в браузере
                        телефона на Android не открывается, а скачивается файлом. Текст собран из этого
                        же файла автоматически ({{ $textBlocks }} блоков, обновлён {{ $textUpdatedAt }}),
                        поэтому редакции не расходятся. После замены файла загляните на страницу и
                        проверьте её глазами.
                    </p>
                @else
                    <p class="rounded-lg bg-warning-50 p-3 text-sm text-warning-700 dark:bg-warning-500/10 dark:text-warning-300">
                        Клиенты читают <b>текстовую версию</b> договора на странице сайта: PDF в браузере
                        телефона на Android не открывается, а скачивается файлом. Текст из этого файла
                        прочитать не удалось (так бывает со сканами и защищёнными PDF), поэтому на
                        странице стоит прежняя редакция. Загрузите файл, сохранённый из Word, — тогда
                        страница пересоберётся сама.
                    </p>
                @endif
            </div>
        @else
