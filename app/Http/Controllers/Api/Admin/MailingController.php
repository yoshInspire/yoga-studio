 ...
 * экране справочный.
 *
 * **Отправка асинхронная.** Маршрут только ставит задания в очередь и
 * отвечает сразу — ждать семь десятков SMTP-сессий в одном запросе нельзя:
 * 06.08.2026 запрос отваливался по таймауту, администратор считал, что
 * рассылка не ушла, и жал «Отправить» ещё раз.
 */
class MailingController extends Controller
{
    ...
    public function sendWeekly(Request $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'message' => $result['sent'] === 0
                ...
                : 'Анонс уходит '.$result['sent'].' получателям, это занимает несколько минут.',
            'state' => $this->payload(),
        ]);
    }
     ...
     *
     * Отвечает сразу: сообщения ставятся в очередь и расходятся в фоне.
     * Повтор с тем же текстом никому не пошлёт вторую копию — тогда
     * `queued` равен нулю, и экран говорит, что рассылка уже была.
     *
     * Ключ `sent` оставлен намеренно: его читают сборки приложения, уже
     * выложенные в EAS.
     */
    public function sendCustom(Request $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'queued' => $queued,
            'skipped' => $result['skipped'],
            'already_sent' => $queued === 0,
            'message' => $queued === 0
                ? 'Это сообщение уже отправлено — второй копии не будет.'
                : 'Оповещение уходит '.$queued.' получателям, это занимает несколько минут.',
        ]);
    }
